...
BEGIN
  ...
  SELECT
    ...
    (SELECT COUNT(*) FROM public.messages m WHERE m.lead_id = l.id AND m.direction = 'inbound' AND m.created_at::date = target_date),
    (SELECT COUNT(*) FROM public.messages m WHERE m.lead_id = l.id AND m.direction = 'outbound' AND m.created_at::date = target_date),
    (SELECT COUNT(*) FROM public.messages m WHERE m.lead_id = l.id AND m.sender_type = 'template' AND m.created_at::date = target_date),
    (SELECT COUNT(*) FROM public.conversation_events e WHERE e.lead_id = l.id AND e.event_type = 'followup_sent' AND e.created_at::date = target_date)
  FROM public.leads l
